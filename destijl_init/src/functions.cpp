    ...
    while (1) {
        ...
        err = receive_message_from_monitor(msg.header, msg.data);
         if (strcmp(msg.header, HEADER_MTS_DMB_ORDER) == 0) {
             if (msg.data[0] == DMB_START_WITHOUT_WD) {
...
void f_niveauBatterie (void *arg)
{
	...
	while(1){
         rt_task_wait_period(NULL);
         printf("%s: stop period\n", info.name);
   	 rt_mutex_acquire(&mutex_robotStarted, TM_INFINITE);
   	 	if(robotStarted == 1){   	 
                /*tensionBatterie = send_command_to_robot(DMB_GET_VBAT);
                printf("%s: data done\n", info.name);
                  set_msgToMon_header(&msg, HEADER_STM_BAT);
                printf("%s: header msg done\n", info.name);
                tensionBatterie += '0';
                 set_msgToMon_data(&msg,&tensionBatterie);
                printf("%s: data msg done\n", info.name);
   		 write_in_queue(&q_messageToMon, msg);*/
		}
         rt_mutex_release(&mutex_robotStarted);
	}
...
void f_perte_info(void *arg){
    ...
    printf("Init %s\n", info.name);
    rt_sem_p(&sem_barrier, TM_INFINITE);
    rt_sem_p(&sem_pertInfo, TM_INFINITE);
    while(1){
		rt_mutex_acquire(&mutex_etat_communication, TM_INFINITE);
